...
#   4. Known Bluetooth LE anchor map (optional; when BLE was requested)
#   5. Native OS location provider
#        macOS: CoreWLAN for Wi-Fi + Core Location for position
#        Linux: NetworkManager nmcli (preferred), iw fallback, GeoClue helper
#   6. Public-IP geolocation fallback
#
...
# by the script itself. Linux BLE uses BlueZ bluetoothctl when available;
# macOS controller evidence uses IOBluetoothHostController with an
# IOBluetoothHCIController IORegistry fallback; BLE discovery uses built-in
# CoreBluetooth through osascript/JXA. Other optional OS tooling is detected
# at runtime.
#
...
usage() {
    cat <<'EOF'
Usage:
  get-device-coords-v2.6.sh [options]
...
Options:
  ...
  --bluetooth-map PATH      Optional v3.62-compatible JSON anchor map.
                            Matching supports Address, ServiceUuid, or
                            ManufacturerId+Name.
  --bluetooth-scan-seconds N
                            Bluetooth LE advertisement discovery window (1-30).
                            Linux uses BlueZ bluetoothctl; macOS uses native
                            IOBluetooth HCI controller evidence plus CoreBluetooth
                            discovery through built-in osascript/JXA.
...
Examples:
  ...
  GOOGLE_GEO_API_KEY='...' ./get-device-coords-v2.6.sh

  WIGLE_API_NAME='...' WIGLE_API_TOKEN='...' ./get-device-coords-v2.6.sh

  ./get-device-coords-v2.6.sh --wifi-map ./Corporate-WifiLocations.csv
...
Notes:
  ...
  * BLE scanning is opt-in. Linux uses BlueZ bluetoothctl. macOS queries the
    public IOBluetoothHostController for local HCI controller evidence and uses
    CoreBluetooth through osascript/JXA for LE discovery. If the public bridge
    is unavailable, IOBluetoothHCIController IORegistry presence is used as a
    controller-detection fallback. No pairing, connection, authentication,
    GATT interrogation, or observation upload is performed.
...
if [ -n "$BLUETOOTH_MAP_PATH" ] && [ ! -f "$BLUETOOTH_MAP_PATH" ]; then
    echo "ERROR: --bluetooth-map path does not exist or is not a file [$BLUETOOTH_MAP_PATH]." >&2
    exit 2
fi
...
LINUX_GEOCLUE_UNAVAILABLE_DETAIL=""
...
describe_geoclue_unavailable() {
    if [ -x /usr/libexec/geoclue ] || [ -x /usr/lib/geoclue ] ||
       [ -x /usr/libexec/geoclue-2.0/geoclue ]; then
        printf 'GeoClue service components are present, but the where-am-i demo/helper is not installed or discoverable.'
        return
    fi

    if command -v busctl >/dev/null 2>&1 &&
       busctl --system list 2>/dev/null |
           grep -q 'org\.freedesktop\.GeoClue2'; then
        printf 'GeoClue is registered on the system D-Bus, but the where-am-i demo/helper is not installed or discoverable.'
        return
    fi

    printf 'GeoClue/where-am-i was not found on this host.'
}
...
resolve_linux_native_location() {
    ...
    if [ -z "$helper" ]; then
        LINUX_NATIVE_LOCATION_STATUS="UnavailableNoHelper"
        [ -n "$LINUX_GEOCLUE_UNAVAILABLE_DETAIL" ] ||
            LINUX_GEOCLUE_UNAVAILABLE_DETAIL="$(describe_geoclue_unavailable)"
        return 1
    fi
...
fetch_url() {
    local url="$1" output="$2" request_timeout
    request_timeout="${3:-6}"

    if command -v curl >/dev/null 2>&1; then
        curl -sS --connect-timeout 2 --max-time "$request_timeout" "$url" -o "$output"
        return $?
    fi

    if command -v wget >/dev/null 2>&1; then
        wget -q -T "$request_timeout" -O "$output" "$url"
        return $?
    fi
...
location_confidence() {
    ...
    if [ "$method" = "PublicIP" ]; then
        printf 'Low'
    elif [ "$method" = "KnownWiFi" ] || [ "$method" = "KnownBluetooth" ]; then
        printf 'High'
...
collect_current_wifi_linux() {
    local line rest iw_link iw_bssid
    ...
    # nmcli exposes a normalized signal percentage but not an actual dBm value.
    # When iw is available, enrich the current association with the kernel link
    # RSSI without requiring another active scan.
    if command -v iw >/dev/null 2>&1 && [ -n "$DEVICE_WIFI_INTERFACE" ]; then
        iw_link="$(
            LC_ALL=C LANG=C iw dev "$DEVICE_WIFI_INTERFACE" link 2>/dev/null || true
        )"
        iw_bssid="$(
            printf '%s\n' "$iw_link" |
                sed -n 's/^[[:space:]]*Connected to[[:space:]]*//p' |
                grep -Eo '^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}' |
                head -n 1
        )"
        iw_bssid="$(normalize_bssid "$iw_bssid")"

        if [ -n "$iw_bssid" ] && [ "$iw_bssid" = "$CURRENT_WIFI_BSSID" ]; then
            CURRENT_WIFI_SIGNAL_DBM="$(
                printf '%s\n' "$iw_link" |
                    sed -n 's/^[[:space:]]*signal:[[:space:]]*\(-\{0,1\}[0-9][0-9]*\) dBm.*/\1/p' |
                    head -n 1
            )"
        fi
    fi
...
resolve_public_ip() {
    ...
    if [ -n "$PUBLIC_IP_LAT" ] && [ -n "$PUBLIC_IP_LON" ]; then
        verbose "Public-IP resolver: completed for ${PUBLIC_IP:-unknown}; provider=${PUBLIC_IP_ISP:-NotReported}; ASN=${PUBLIC_IP_ASN:-NotReported}."
        return 0
    fi
    return 1
}
...
probe_router_web_identity() {
    local headers body title server auth location tls_raw tls_err cert_file
    ...
    if command -v openssl >/dev/null 2>&1; then
        # A filtered/black-holed TCP/443 gateway must never stall the whole run.
        # Use one bounded TLS connection and parse the certificate locally rather
        # than opening a second s_client session when the first parser is empty.
        tls_raw="$WORK_DIR/router-tls.txt"
        tls_err="$WORK_DIR/router-tls.err"
        cert_file="$WORK_DIR/router-tls-cert.pem"
        : > "$tls_raw"
        : > "$tls_err"
        : > "$cert_file"

        run_with_timeout 3 "$tls_raw" "$tls_err" \
            sh -c 'exec openssl s_client -connect "$1:443" -servername "$1" -showcerts </dev/null' \
            sh "$DEFAULT_GATEWAY" || true

        ROUTER_TLS_SUBJECT="$(
            sed -n 's/^subject[[:space:]]*=[[:space:]]*//p' "$tls_raw" 2>/dev/null |
            head -n 1
        )"

        if [ -z "$ROUTER_TLS_SUBJECT" ]; then
            awk '/-----BEGIN CERTIFICATE-----/{capture=1} capture{print} /-----END CERTIFICATE-----/{exit}' \
                "$tls_raw" > "$cert_file" 2>/dev/null || true
            if [ -s "$cert_file" ]; then
                ROUTER_TLS_SUBJECT="$(
...
get_ip_geo_line() {
    ...
    fetch_url "https://ipwho.is/${ip}" "$response" 3 >/dev/null 2>&1 || return 1
...
TRACE_FILE=""
TRACE_GEO=""
TRACE_ENGINE="Unavailable"

collect_linux_route_trace_with_ping() {
    local raw="$1" ttl probe hop
    command -v ping >/dev/null 2>&1 || return 1

    : > "$raw"
    ttl=1
    while [ "$ttl" -le 8 ]; do
        probe="$WORK_DIR/ping-ttl-${ttl}.txt"

        run_with_timeout 3 "$probe" "$WORK_DIR/ping-ttl-${ttl}.err" \
            ping -4 -n -c 1 -W 1 -t "$ttl" 1.1.1 || true

        hop="$(
            grep -Ei '(^|[[:space:]])(From|from)[[:space:]]' "$probe" 2>/dev/null |
...
collect_route_trace() {
    local raw line ip scope geo city region country isp asn hopn public_geo_lookups
    TRACE_FILE="$WORK_DIR/trace.tsv"
    : > "$TRACE_FILE"
    raw="$WORK_DIR/traceroute.txt"
    : > "$raw"
    TRACE_ENGINE="Unavailable"
    public_geo_lookups=0

    verbose "Route trace: collecting bounded route evidence to 1.1.1.1."
    ...
    if [ "$PLATFORM" = "Linux" ]; then
        if command -v traceroute >/dev/null 2>&1; then
            ...
            if route_trace_raw_has_hops "$raw"; then
                TRACE_ENGINE="traceroute"
            fi
        fi

        if [ "$TRACE_ENGINE" = "Unavailable" ] &&
           command -v tracepath >/dev/null 2>&1; then
            : > "$raw"
            run_with_timeout 12 "$raw" "$WORK_DIR/traceroute.err" \
                tracepath -n -m 8 1.1.1 || true
            if route_trace_raw_has_hops "$raw"; then
                TRACE_ENGINE="tracepath"
            fi
        fi

        if [ "$TRACE_ENGINE" = "Unavailable" ] &&
           command -v busybox >/dev/null 2>&1 &&
           busybox traceroute --help >/dev/null 2>&1; then
            : > "$raw"
            run_with_timeout 12 "$raw" "$WORK_DIR/traceroute.err" \
                busybox traceroute -4 -n -m 8 -w 1 1.1.1 || true
            if route_trace_raw_has_hops "$raw"; then
                TRACE_ENGINE="BusyBoxTraceroute"
            fi
        fi

        if [ "$TRACE_ENGINE" = "Unavailable" ]; then
            : > "$raw"
            if collect_linux_route_trace_with_ping "$raw" &&
               route_trace_raw_has_hops "$raw"; then
                TRACE_ENGINE="PingTTL"
            fi
        fi

        if [ "$TRACE_ENGINE" = "Unavailable" ]; then
            verbose "Route trace unavailable: none of traceroute, tracepath, BusyBox traceroute, or TTL-limited ping produced hop evidence."
            return 1
        fi
    else
        if [ -x /usr/sbin/traceroute ]; then
            ...
            : > "$raw"
            run_with_timeout 12 "$raw" "$WORK_DIR/traceroute.err" \
                traceroute -n -m 8 -w 1 1.1.1 || true
            if route_trace_raw_has_hops "$raw"; then
                TRACE_ENGINE="traceroute"
            fi
        fi
        ...
        if [ "$TRACE_ENGINE" = "Unavailable" ]; then
            verbose "Route trace unavailable: traceroute was not found or produced no hop evidence on macOS."
            return 1
        fi
    fi
    ...
    while IFS= read -r line; do
        ...
        # tracepath : " 1?: [LOCALHOST] ..." / " 1: 192.0.2.1 ..."
        # PingTTL   : synthetic "1  192.0.2.1"
        printf '%s\n' "$line" |
            grep -Eq '^[[:space:]]*[0-9]+([?]?:|[[:space:]])' || continue
        ...
        if [ "$scope" = "Public" ]; then
            # Router/VPN inference only needs the first two public transit hops.
            # Avoid serial geolocation of every hop, which can add many seconds
            # when an external lookup is slow or blocked.
            if [ "$ip" = "1.1.1.1" ]; then
                geo="$(get_ip_geo_line "$ip" || true)"
            elif [ "$public_geo_lookups" -lt 2 ]; then
                geo="$(get_ip_geo_line "$ip" || true)"
                public_geo_lookups=$((public_geo_lookups + 1))
            else
                geo=""
            fi
...
EOF
    ...
    verbose "Router VPN assessment: no public router WAN was exposed; route evidence is required."
...
BLUETOOTH_SCAN_ENGINE="None"
BLUETOOTH_SCAN_DETAIL=""
BLUETOOTH_EVIDENCE=""
BLUETOOTH_CONTROLLER_STATUS="NotApplicable"
BLUETOOTH_CONTROLLER_SOURCE="None"
BLUETOOTH_CONTROLLER_POWER="NotReported"
BLUETOOTH_CONTROLLER_NAME="NotReported"
BLUETOOTH_CONTROLLER_ADDRESS="NotReported"
BLUETOOTH_CONTROLLER_DETAIL=""
BLUETOOTH_DEVICE_COUNT=0
BLUETOOTH_ANCHOR_COUNT=0
BLUETOOTH_STRONGEST_NAME=""
BLUETOOTH_STRONGEST_ADDRESS=""
BLUETOOTH_STRONGEST_RSSI=""
BLUETOOTH_LOCATION_MATCH="NotConfigured"
BLUETOOTH_LOCATION_DISTANCE="NotApplicable"
BLUETOOTH_ANCHOR_LAT=""
BLUETOOTH_ANCHOR_LON=""
BLUETOOTH_ANCHOR_SITE=""
BLUETOOTH_ANCHOR_MATCHED_BY=""
BLUETOOTH_ANCHOR_RSSI=""
BLUETOOTH_ANCHOR_MAP_URI=""
BLUETOOTH_DEVICE_FILE="$WORK_DIR/bluetooth-devices.tsv"
BLUETOOTH_ANCHOR_FILE="$WORK_DIR/bluetooth-anchors.tsv"
: > "$BLUETOOTH_DEVICE_FILE"
: > "$BLUETOOTH_ANCHOR_FILE"

bluetooth_company_name() {
    case "${1:-}" in
        6) printf 'Microsoft' ;;
        10) printf 'Qualcomm' ;;
        13) printf 'Texas Instruments' ;;
        15) printf 'Broadcom' ;;
        48) printf 'STMicroelectronics' ;;
        76) printf 'Apple' ;;
        89) printf 'Nordic Semiconductor' ;;
        117) printf 'Samsung' ;;
        224) printf 'Google' ;;
        240) printf 'Garmin' ;;
        301) printf 'Sony' ;;
        658) printf 'Fitbit' ;;
        741) printf 'Amazon' ;;
        65535) printf 'Reserved/Test' ;;
        '') printf '' ;;
        *) printf 'CompanyId:%s' "$1" ;;
    esac
}

bluetooth_primary_manufacturer_id() {
    printf '%s' "${1:-}" |
        tr ', ' ';;' |
        awk -F';' '{for(i=1;i<=NF;i++) if($i ~ /^[0-9]+$/){print $i; exit}}'
}

bluetooth_service_summary() {
    local raw token short out="" count=0 label
    raw="${1:-}"
    [ -n "$raw" ] || return 0

    printf '%s\n' "$raw" | tr ', ' ';;' | tr ';' '\n' |
        while IFS= read -r token; do
            token="$(printf '%s' "$token" | tr '[:lower:]' '[:upper:]' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')"
            [ -n "$token" ] || continue
            short=""
            case "$token" in
                0000????-0000-1000-8000-00805F9B34FB) short="$(printf '%s' "$token" | cut -c5-8)" ;;
                ????) short="$token" ;;
            esac
            case "$short" in
                1800) label="GenericAccess" ;;
                1801) label="GenericAttribute" ;;
                180A) label="DeviceInfo" ;;
                180D) label="HeartRate" ;;
                180F) label="Battery" ;;
                1812) label="HID" ;;
                181A) label="Environmental" ;;
                181C) label="UserData" ;;
                FEAA) label="Eddystone" ;;
                FE2C) label="FastPair" ;;
                FD6F) label="ExposureNotif" ;;
                '') label="$(printf '%s' "$token" | cut -c1-8)" ;;
                *) label="$short" ;;
            esac
            printf '%s\n' "$label"
        done |
        awk '!seen[$0]++ { if(n++) printf ","; printf "%s",$0; if(n>=3) exit }'
}

bluetooth_sha10() {
    local input="$1" hash=""
    if command -v sha256sum >/dev/null 2>&1; then
        hash="$(printf '%s' "$input" | sha256sum 2>/dev/null | awk '{print toupper(substr($1,1,10))}')"
    elif command -v shasum >/dev/null 2>&1; then
        hash="$(printf '%s' "$input" | shasum -a 256 2>/dev/null | awk '{print toupper(substr($1,1,10))}')"
    elif command -v openssl >/dev/null 2>&1; then
        hash="$(printf '%s' "$input" | openssl dgst -sha256 2>/dev/null | awk '{print toupper(substr($NF,1,10))}')"
    fi
    [ -n "$hash" ] && printf '%s' "$hash" || printf 'UNAVAILABLE'
}

bluetooth_cached_oui_vendor() {
    local address="$1" oui line vendor
    address="$(normalize_bssid "$address")"
    printf '%s' "$address" | grep -Eq '^([0-9A-F]{2}:){5}[0-9A-F]{2}$' || return 1
    oui="$(normalize_oui "$address")"
    line="$(awk -F '|' -v o="$oui" '$1==o && $2!="<none>" {print; exit}' "$OUI_CACHE_FILE" 2>/dev/null || true)"
    [ -n "$line" ] || return 1
    IFS='|' read -r _ vendor _ <<EOF_CACHE
$line
EOF_CACHE
    [ -n "$vendor" ] && printf '%s' "$vendor"
}

bluetooth_address_is_locally_administered() {
    local address="$1" first
    address="$(normalize_bssid "$address")"
    printf '%s' "$address" | grep -Eq '^([0-9A-F]{2}:){5}[0-9A-F]{2}$' || return 0
    first="${address%%:*}"
    awk -v h="$first" '
        function hv(c) {
            c=toupper(c)
        ...
        BEGIN {
            n=(hv(substr(h,1,1))*16)+hv(substr(h,2,1))
            exit !((n % 4) >= 2)
        }
    '
}

bluetooth_vendor_information() {
    local address="$1" addr_type="$2" company="$3" vendor=""
    case "$address" in
        CBUUID:*)
            [ -n "$company" ] && printf '%s; CoreBluetooth identifier (MAC not exposed)' "$company" || printf 'CoreBluetooth identifier (MAC not exposed)'
            return
            ;;
    esac

    if printf '%s' "$addr_type" | grep -Eqi 'random'; then
        printf '<random>'
        return
    fi

    if bluetooth_address_is_locally_administered "$address"; then
        printf 'Private'
        return
    fi

    vendor="$(bluetooth_cached_oui_vendor "$address" 2>/dev/null || true)"
    if [ -n "$vendor" ]; then
        printf '%s' "$vendor"
    else
        printf 'Unknown'
...
bluetooth_identity_confidence() {
    local addr_type="$1" name="$2" mfr="$3" services="$4" vendor="$5"
    local has_name=0 has_mfr=0 has_svc=0 is_random=0 has_vendor=0
    [ -n "$name" ] && [ "$name" != "<unknown>" ] && has_name=1
    [ -n "$mfr" ] && has_mfr=1
    [ -n "$services" ] && has_svc=1
    printf '%s' "$addr_type" | grep -Eqi 'random' && is_random=1 || true
    case "$vendor" in ''|'Unknown'|'Private'|'<random>'|*'MAC not exposed'*) ;; *) has_vendor=1 ;; esac

    if { [ "$has_mfr" -eq 1 ] && [ "$has_svc" -eq 1 ]; } ||
       { [ "$has_name" -eq 1 ] && { [ "$has_mfr" -eq 1 ] || [ "$has_svc" -eq 1 ]; }; }; then
        printf 'High'
    elif [ "$has_mfr" -eq 1 ] || [ "$has_svc" -eq 1 ] || { [ "$has_vendor" -eq 1 ] && [ "$has_name" -eq 1 ]; }; then
        printf 'Medium'
    elif [ "$is_random" -eq 1 ] && [ "$has_name" -eq 0 ] && [ "$has_mfr" -eq 0 ] && [ "$has_svc" -eq 0 ]; then
        printf 'Ephemeral'
    elif [ "$has_name" -eq 1 ] || [ "$has_vendor" -eq 1 ]; then
        printf 'Low'
    else
        printf 'Ephemeral'
    fi
...
parse_linux_ble_scan() {
    ...
    awk '
        ...
        function hex2dec(h, i,n) {
            sub(/^0[xX]/,"",h); n=0
            for(i=1;i<=length(h);i++) n=n*16+hexval(substr(h,i,1))
            return n
        }
        function signed32(n) {
            return (n >= 2147483648 ? n - 4294967296 : n)
        }
        function parse_rssi(v, m,n) {
            v=clean(v)

            # Newer bluetoothctl builds can render signed D-Bus values as both
            # hexadecimal and a parenthesized decimal, for example:
            #   RSSI: 0xffffffb2 (-78)
            # Prefer the explicit signed decimal when it is present.
            if(match(v,/\(-[0-9]+\)/)) {
                m=substr(v,RSTART+1,RLENGTH-2)
                return m+0
            }
            if(match(v,/\([0-9]+\)/)) {
                m=substr(v,RSTART+1,RLENGTH-2)
            ...
            if(match(v,/^0[xX][0-9A-Fa-f]+/)) {
                n=hex2dec(substr(v,RSTART,RLENGTH))
                return signed32(n)
            }
            return -999
        }
        function adduniq(old,val, arr,n,i) {
            if(val=="") return old
            n=split(old,arr,";")
            for(i=1;i<=n;i++) if(toupper(arr[i])==toupper(val)) return old
            return old=="" ? val : old ";" val
        }
        function clean(v) { gsub(/[|\r\n\t]/," ",v); gsub(/[ ]+/," ",v); gsub(/^[ ]+|[ ]+$/,"",v); return v }
        function usable_label(v) {
            v=clean(v)
        ...
        }
        {
            ...
            esc=sprintf("%c",27)
            gsub(esc "\\[[0-9;?]*[ -/]*[@-~]", "", line)
            if(!match(line, /[0-9A-Fa-f][0-9A-Fa-f]:[0-9A-Fa-f][0-9A-Fa-f]:[0-9A-Fa-f][0-9A-Fa-f]:[0-9A-Fa-f][0-9A-Fa-f]:[0-9A-Fa-f][0-9A-Fa-f]:[0-9A-Fa-f][0-9A-Fa-f]/)) next
            mac=toupper(substr(line,RSTART,RLENGTH))
            if(!(mac in present)) {
                present[mac]=1
                order[++count]=mac
                rssi[mac]=-999
            ...
            if(rest ~ /^\(public\)/) addrtype[mac]="Public"
            else if(rest ~ /^\(random\)/) addrtype[mac]="Random"
            else if(rest ~ /^AddressType:[ \t]*/) {
                x=rest
                sub(/^AddressType:[ \t]*/,"",x)
                split(x,a,/[^A-Za-z]/)
                if(a[1]!="") addrtype[mac]=a[1]
            }
            ...
            if(p>0) {
                ...
                v=parse_rssi(x)
                if(v!=-999) {
                    if(rssi[mac]==-999 || v>rssi[mac]) rssi[mac]=v
                    seen[mac]++
                }
            }

            if(index(line,"ManufacturerData Key:")>0) {
                x=substr(line,index(line,"ManufacturerData Key:")+21)
                sub(/^[ \t]+/,"",x)
                if(match(x,/0[xX][0-9A-Fa-f]+/)) mfr[mac]=adduniq(mfr[mac],hex2dec(substr(x,RSTART,RLENGTH)))
            }

            if(index(line,"UUIDs:")>0 || index(line,"UUID:")>0 || index(line,"ServiceData Key:")>0) {
                x=rest
                while(match(x,/[0-9A-Fa-f][0-9A-Fa-f][0-9A-Fa-f][0-9A-Fa-f]([0-9A-Fa-f-]{4,32})?/)) {
                    u=toupper(substr(x,RSTART,RLENGTH))
                    if(length(u)==4 || length(u)==36) svc[mac]=adduniq(svc[mac],u)
                    x=substr(x,RSTART+RLENGTH)
                }
            }

            # Keep the BlueZ identity properties separately.  Alias is the
            # user-facing Device1 identity, Name is the remote-advertised name,
            # and the bare [NEW] discovery label is only a last-resort fallback.
            if(rest ~ /^Alias:[ \t]*/) {
                x=rest
                sub(/^Alias:[ \t]*/,"",x)
                x=clean(x)
                if(usable_label(x)) alias[mac]=x
            } else if(rest ~ /^Name:[ \t]*/) {
                x=rest
                sub(/^Name:[ \t]*/,"",x)
                x=clean(x)
                if(usable_label(x)) explicit_name[mac]=x
            } else if(line ~ /\[(NEW|CHG)\][ \t]+Device[ \t]+/ &&
                      rest !~ /^(RSSI:|TxPower:|ManufacturerData|ServiceData|UUIDs?:|ServicesResolved:|Paired:|Connected:|Trusted:|Blocked:|WakeAllowed:|LegacyPairing:|Modalias:|Class:|Icon:|AddressType:|\()/) {
                x=clean(rest)
                if(usable_label(x)) discovery_label[mac]=x
            }
        }
        END {
            for(i=1;i<=count;i++) {
                mac=order[i]
                if(seen[mac]<1) seen[mac]=1
                rv=(rssi[mac]==-999 ? "" : rssi[mac])
                label=usable_label(alias[mac]) ? alias[mac] : (usable_label(explicit_name[mac]) ? explicit_name[mac] : (usable_label(discovery_label[mac]) ? discovery_label[mac] : "<unknown>"))
                printf "%s|%s|%s|%s|%s|%s|%s|%d|BlueZBluetoothctl\n", mac,clean(label),rv,addrtype[mac],advtype[mac],mfr[mac],svc[mac],seen[mac]
            }
        }
    ' "$input" |
        awk -F'|' '{ key=($3=="" ? -999 : $3); printf "%d\t%s\n", key, $0 }' |
        sort -k1,1nr -k2,2 |
        cut -f2-
}

scan_bluetooth_linux() {
    local seconds="$1" ctl show raw rc count scan_posture
    ctl="$(command -v bluetoothctl 2>/dev/null || true)"
    if [ -z "$ctl" ]; then
        BLUETOOTH_SCAN_STATUS="UnavailableNoScanner"
        BLUETOOTH_SCAN_ENGINE="None"
        BLUETOOTH_SCAN_DETAIL="BlueZ bluetoothctl was not found; passive Linux BLE advertisement scanning could not run."
        return
    fi

    show="$(LC_ALL=C "$ctl" show 2>&1 || true)"
    if printf '%s' "$show" | grep -Eqi 'No default controller|No controller available'; then
        BLUETOOTH_SCAN_STATUS="UnavailableNoAdapter"
        BLUETOOTH_SCAN_ENGINE="BlueZBluetoothctl"
        BLUETOOTH_SCAN_DETAIL="BlueZ is available, but no Bluetooth controller is available to bluetoothctl."
        return
    fi
    if printf '%s' "$show" | grep -Eqi 'Powered:[[:space:]]+no'; then
        BLUETOOTH_SCAN_STATUS="PoweredOff"
        BLUETOOTH_SCAN_ENGINE="BlueZBluetoothctl"
        BLUETOOTH_SCAN_DETAIL="The Linux Bluetooth controller is powered off; the script did not change controller state."
        return
    fi

    if printf '%s' "$show" | grep -Eqi 'Discoverable:[[:space:]]+yes'; then
        scan_posture="ActiveScanPossible"
    else
        scan_posture="Passive"
    fi

    raw="$WORK_DIR/bluetooth-linux.raw"
    verbose "Bluetooth: starting Linux BlueZ LE discovery for ${seconds} second(s); scanPosture=${scan_posture}; no pairing or connection will be attempted."
    LC_ALL=C "$ctl" --timeout "$seconds" scan le >"$raw" 2>&1
    rc=$?
    "$ctl" scan off >/dev/null 2>&1 || true

    parse_linux_ble_scan "$raw" > "$BLUETOOTH_DEVICE_FILE"
    count="$(awk 'END{print NR+0}' "$BLUETOOTH_DEVICE_FILE" 2>/dev/null || printf '0')"
    verbose "Bluetooth: BlueZ parser normalized signed/hex RSSI and selected display identity in priority order Alias -> Name -> discovery label; parsedDevices=${count}."

    if printf '%s\n' "$(cat "$raw" 2>/dev/null)" | grep -Eqi 'not authorized|permission denied|access denied|org\.bluez\.Error\.NotAuthorized'; then
        BLUETOOTH_SCAN_STATUS="AccessDenied"
        BLUETOOTH_SCAN_ENGINE="BlueZBluetoothctl"
        BLUETOOTH_SCAN_DETAIL="BlueZ denied BLE discovery for the current user/session. No pairing or connection was attempted."
    elif printf '%s\n' "$(cat "$raw" 2>/dev/null)" | grep -Eqi 'No default controller|No controller available'; then
        BLUETOOTH_SCAN_STATUS="UnavailableNoAdapter"
        BLUETOOTH_SCAN_ENGINE="BlueZBluetoothctl"
        BLUETOOTH_SCAN_DETAIL="BlueZ reported no usable Bluetooth controller."
    elif [ "$rc" -ne 0 ] && [ "$count" -eq 0 ]; then
        BLUETOOTH_SCAN_STATUS="Failed"
        BLUETOOTH_SCAN_ENGINE="BlueZBluetoothctl"
        BLUETOOTH_SCAN_DETAIL="BlueZ bluetoothctl LE scan failed with exit code ${rc}."
    else
        BLUETOOTH_SCAN_STATUS="Success"
        BLUETOOTH_SCAN_ENGINE="BlueZBluetoothctl"
        if [ "$scan_posture" = "Passive" ]; then
            BLUETOOTH_SCAN_DETAIL="Passive Linux BLE discovery completed through BlueZ for ${seconds} second(s); unique advertisers=${count}. No pairing, connection, authentication, or GATT interrogation was performed."
        else
            BLUETOOTH_SCAN_DETAIL="Linux BLE discovery completed through BlueZ for ${seconds} second(s); unique advertisers=${count}. The local adapter was already discoverable, so BlueZ may issue active scan requests; the script did not change controller state and performed no pairing, connection, authentication, or GATT interrogation."
        fi
    fi
}
...
function gdcClean(value) {
    if (value === null || value === undefined) return '';
    return String(value).replace(/[|\r\n\t]/g, ' ').replace(/^\s+|\s+$/g, '');
}

function gdcObjCString(value) {
    if (!value) return '';
    try { return gdcClean(ObjC.unwrap(value)); } catch (e) { return ''; }
}

function run() {
    try {
        var controller = $.IOBluetoothHostController.defaultController;
        if (!controller) {
            return 'STATUS|UnavailableNoAdapter|IOBluetoothHostController|NotReported|NotReported|NotReported|No default IOBluetoothHostController was returned.';
        }

        var powerRaw = -1;
        try { powerRaw = Number(controller.powerState); } catch (e) {}
        var powerText = 'Unknown';
        if (powerRaw === 1) powerText = 'On';
        else if (powerRaw === 0) powerText = 'Off';
        else if (powerRaw === 255) powerText = 'Uninitialized';
        else if (powerRaw >= 0) powerText = 'Unknown(' + powerRaw + ')';

        var name = '';
        var address = '';
        try { name = gdcObjCString(controller.nameAsString); } catch (e) {}
        try { address = gdcObjCString(controller.addressAsString); } catch (e) {}
        if (address) address = address.replace(/-/g, ':').toUpperCase();

        if (!name) name = 'NotReported';
        if (!address) address = 'NotReported';

        return 'STATUS|Available|IOBluetoothHostController|' + powerText + '|' + name + '|' + address + '|Public IOBluetooth host-controller query succeeded.';
    } catch (e) {
        return 'STATUS|Failed|IOBluetoothHostController|NotReported|NotReported|NotReported|' + gdcClean(e.toString());
    }
}
JXA
}

probe_macos_bluetooth_controller() {
    local osa script err output status source power name address detail ioreg_bin ioreg_text

    BLUETOOTH_CONTROLLER_STATUS="Unknown"
    BLUETOOTH_CONTROLLER_SOURCE="None"
    BLUETOOTH_CONTROLLER_POWER="NotReported"
    BLUETOOTH_CONTROLLER_NAME="NotReported"
    BLUETOOTH_CONTROLLER_ADDRESS="NotReported"
    ...
    if [ -n "$osa" ]; then
        script="$WORK_DIR/bluetooth-macos-hci.jxa"
        err="$WORK_DIR/bluetooth-macos-hci.err"
        write_macos_hci_jxa "$script"
        output="$("$osa" -l JavaScript "$script" 2>"$err" || true)"
        status="$(printf '%s\n' "$output" | awk -F'|' '$1=="STATUS" {print $2; exit}')"
        source="$(printf '%s\n' "$output" | awk -F'|' '$1=="STATUS" {print $3; exit}')"
        power="$(printf '%s\n' "$output" | awk -F'|' '$1=="STATUS" {print $4; exit}')"
        name="$(printf '%s\n' "$output" | awk -F'|' '$1=="STATUS" {print $5; exit}')"
        address="$(printf '%s\n' "$output" | awk -F'|' '$1=="STATUS" {print $6; exit}')"
        detail="$(printf '%s\n' "$output" | awk -F'|' '$1=="STATUS" {sub(/^[^|]*\|[^|]*\|[^|]*\|[^|]*\|[^|]*\|[^|]*\|/,""); print; exit}')"

        if [ -n "$status" ]; then
            BLUETOOTH_CONTROLLER_STATUS="$status"
            BLUETOOTH_CONTROLLER_SOURCE="${source:-IOBluetoothHostController}"
            BLUETOOTH_CONTROLLER_POWER="${power:-NotReported}"
            BLUETOOTH_CONTROLLER_NAME="${name:-NotReported}"
            BLUETOOTH_CONTROLLER_ADDRESS="${address:-NotReported}"
            BLUETOOTH_CONTROLLER_DETAIL="${detail:-IOBluetooth host-controller query returned no detail.}"
        fi
    fi

    # IOBluetoothHCIController is the lower-level IORegistry/kernel object. Use it
    # only as a presence fallback when the public IOBluetoothHostController bridge
    # did not confirm a controller; do not issue private HCI commands.
    if [ "$BLUETOOTH_CONTROLLER_STATUS" != "Available" ]; then
        ioreg_bin="$(command -v ioreg 2>/dev/null || true)"
        if [ -n "$ioreg_bin" ]; then
            ioreg_text="$("$ioreg_bin" -r -c IOBluetoothHCIController -l -w 0 2>/dev/null || true)"
            if printf '%s\n' "$ioreg_text" | grep -q 'IOBluetoothHCIController'; then
                if [ "$BLUETOOTH_CONTROLLER_STATUS" = "Unknown" ] || [ "$BLUETOOTH_CONTROLLER_STATUS" = "UnavailableNoAdapter" ] || [ "$BLUETOOTH_CONTROLLER_STATUS" = "Failed" ]; then
                    BLUETOOTH_CONTROLLER_STATUS="DetectedByIORegistry"
                    BLUETOOTH_CONTROLLER_SOURCE="IOBluetoothHCIController/ioreg"
                    BLUETOOTH_CONTROLLER_DETAIL="IOBluetoothHCIController is present in the IORegistry; public IOBluetoothHostController identity was unavailable."
                fi
            fi
        fi
    fi

    verbose "Bluetooth controller: status=$BLUETOOTH_CONTROLLER_STATUS; source=$BLUETOOTH_CONTROLLER_SOURCE; power=$BLUETOOTH_CONTROLLER_POWER; name=$BLUETOOTH_CONTROLLER_NAME; address=$BLUETOOTH_CONTROLLER_ADDRESS."
}
...
function gdcManufacturerId(data) {
    if (!data) return '';
    try {
        var desc = gdcObjCString(data.description);
        var hex = desc.replace(/[^0-9A-Fa-f]/g, '');
        if (hex.length < 4) return '';
        return String(parseInt(hex.substr(2, 2) + hex.substr(0, 2), 16));
    } catch (e) { return ''; }
}

function gdcServiceUuids(advertisementData) {
    var out = [];
    try {
        var values = advertisementData.objectForKey($.CBAdvertisementDataServiceUUIDsKey);
        if (!values) return '';
        var count = Number(values.count);
        for (var i = 0; i < count; i++) {
            var item = values.objectAtIndex(i);
            var text = gdcObjCString(item.UUIDString).toUpperCase();
    ...
    methods: {
        'centralManagerDidUpdateState:': {
            ...
            implementation: function (manager) {
                try { gdcState = Number(manager.state); } catch (e) { gdcState = 0; }
            }
        },
        'centralManager:didDiscoverPeripheral:advertisementData:RSSI:': {
            types: ['void', ['id', 'id', 'id', 'id']],
            implementation: function (manager, peripheral, advertisementData, rssiObject) {
            try {
                var identifier = gdcObjCString(peripheral.identifier.UUIDString).toUpperCase();
                if (!identifier) return;
                var key = identifier;
                var name = '';
                try { name = gdcObjCString(advertisementData.objectForKey($.CBAdvertisementDataLocalNameKey)); } catch (e) {}
                if (!name) {
                    try { name = gdcObjCString(peripheral.name); } catch (e) {}
                }
                if (!name) name = '<unknown>';
                var rssi = Number(ObjC.unwrap(rssiObject));
                var manufacturerId = '';
                try { manufacturerId = gdcManufacturerId(advertisementData.objectForKey($.CBAdvertisementDataManufacturerDataKey)); } catch (e) {}
                var services = gdcServiceUuids(advertisementData);
                var connectable = 'LE';
                try {
                    var c = advertisementData.objectForKey($.CBAdvertisementDataIsConnectable);
                    if (c) connectable = Number(ObjC.unwrap(c)) ? 'Connectable' : 'NonConnectable';
                } catch (e) {}

                if (!gdcDevices[key]) {
                    gdcDevices[key] = { id: identifier, name: name, rssi: rssi, mfr: manufacturerId, services: services, seen: 1, adv: connectable };
                } else {
                    var d = gdcDevices[key];
                    d.seen += 1;
                    if (manufacturerId) d.mfr = manufacturerId;
                    if (services) d.services = services;
                    if (rssi > d.rssi) { d.rssi = rssi; d.name = name; d.adv = connectable; }
                }
            } catch (e) {}
            }
        }
...
function gdcRunLoop(seconds) {
    $.NSRunLoop.currentRunLoop.runUntilDate($.NSDate.dateWithTimeIntervalSinceNow(seconds));
}

function gdcStateName(state) {
    switch (state) {
...
function run(argv) {
    var seconds = Math.max(1, Math.min(30, parseInt(argv[0] || '4', 10)));
    var Delegate = $.GdcBleDelegateV23;
    var delegate = Delegate.alloc.init;
    var manager = $.CBCentralManager.alloc.initWithDelegateQueueOptions(delegate, null, null);

    var readyDeadline = Date.now() + 3000;
    while (Date.now() < readyDeadline) {
        try { gdcState = Number(manager.state); } catch (e) {}
        if (gdcState !== 0 && gdcState !== 1) break;
        gdcRunLoop(0.05);
    }

    try { gdcState = Number(manager.state); } catch (e) {}
    if (gdcState !== 5) {
        return 'STATUS|' + gdcStateName(gdcState) + '|MacOSCoreBluetoothJXA|CoreBluetooth state=' + gdcStateName(gdcState);
    }

    var options = $.NSMutableDictionary.alloc.init;
    options.setObjectForKey($.NSNumber.numberWithBool(true), $.CBCentralManagerScanOptionAllowDuplicatesKey);
    manager.scanForPeripheralsWithServicesOptions(null, options);

    var deadline = Date.now() + (seconds * 1000);
    while (Date.now() < deadline) gdcRunLoop(0.05);
    manager.stopScan;
    gdcRunLoop(0.10);

    var keys = Object.keys(gdcDevices);
    keys.sort(function(a,b) { return gdcDevices[b].rssi - gdcDevices[a].rssi; });
    var lines = ['STATUS|Success|MacOSCoreBluetoothJXA|CoreBluetooth BLE discovery completed; unique advertisers=' + keys.length];
    for (var i = 0; i < keys.length; i++) {
        var d = gdcDevices[keys[i]];
        lines.push('DEVICE|CBUUID:' + gdcClean(d.id) + '|' + gdcClean(d.name) + '|' + d.rssi + '|Opaque|' + gdcClean(d.adv) + '|' + gdcClean(d.mfr) + '|' + gdcClean(d.services) + '|' + d.seen + '|MacOSCoreBluetoothJXA');
    }
    return lines.join('\n');
}
JXA
...
scan_bluetooth_macos() {
    local seconds="$1" osa script output err status engine detail
    probe_macos_bluetooth_controller
    osa="$(command -v osascript 2>/dev/null || true)"
    if [ -z "$osa" ]; then
        BLUETOOTH_SCAN_STATUS="UnavailableNoScanner"
        BLUETOOTH_SCAN_ENGINE="None"
        BLUETOOTH_SCAN_DETAIL="Built-in osascript was not found; macOS CoreBluetooth BLE scanning could not run."
        return
    fi

    script="$WORK_DIR/bluetooth-macos.jxa"
    err="$WORK_DIR/bluetooth-macos.err"
    write_macos_ble_jxa "$script"
    verbose "Bluetooth: starting macOS CoreBluetooth LE advertisement scan for ${seconds} second(s); no pairing or connection will be attempted."
    output="$("$osa" -l JavaScript "$script" "$seconds" 2>"$err" || true)"

    status="$(printf '%s\n' "$output" | awk -F'|' '$1=="STATUS" {print $2; exit}')"
    engine="$(printf '%s\n' "$output" | awk -F'|' '$1=="STATUS" {print $3; exit}')"
    detail="$(printf '%s\n' "$output" | awk -F'|' '$1=="STATUS" {sub(/^[^|]*\|[^|]*\|[^|]*\|/,""); print; exit}')"
    printf '%s\n' "$output" | awk -F'|' '$1=="DEVICE" {sub(/^DEVICE\|/,""); print}' > "$BLUETOOTH_DEVICE_FILE"

    case "$status" in
        Success)
            BLUETOOTH_SCAN_STATUS="Success"
            BLUETOOTH_SCAN_ENGINE="${engine:-MacOSCoreBluetoothJXA}"
            BLUETOOTH_SCAN_DETAIL="${detail}. ScanSeconds=${seconds}. LocalController=${BLUETOOTH_CONTROLLER_STATUS}/${BLUETOOTH_CONTROLLER_SOURCE}; Power=${BLUETOOTH_CONTROLLER_POWER}; Name=${BLUETOOTH_CONTROLLER_NAME}; Address=${BLUETOOTH_CONTROLLER_ADDRESS}. CoreBluetooth reports system-assigned peripheral UUIDs rather than BLE MAC addresses and does not expose a passive/active scan-mode control to this script. No pairing, connection, authentication, or GATT interrogation was performed."
            ;;
        Unauthorized)
            BLUETOOTH_SCAN_STATUS="AccessDenied"
            BLUETOOTH_SCAN_ENGINE="${engine:-MacOSCoreBluetoothJXA}"
            BLUETOOTH_SCAN_DETAIL="macOS denied CoreBluetooth access to the current terminal/automation host. Allow Bluetooth access in Privacy & Security, then rerun."
            ;;
        PoweredOff)
            BLUETOOTH_SCAN_STATUS="PoweredOff"
            BLUETOOTH_SCAN_ENGINE="${engine:-MacOSCoreBluetoothJXA}"
            BLUETOOTH_SCAN_DETAIL="macOS Bluetooth is powered off; the script did not change Bluetooth state."
            ;;
        Unsupported)
            BLUETOOTH_SCAN_STATUS="UnavailableNoAdapter"
            BLUETOOTH_SCAN_ENGINE="${engine:-MacOSCoreBluetoothJXA}"
            BLUETOOTH_SCAN_DETAIL="CoreBluetooth reported that BLE central scanning is unsupported on this Mac."
            ;;
        *)
            BLUETOOTH_SCAN_STATUS="Failed"
            BLUETOOTH_SCAN_ENGINE="${engine:-MacOSCoreBluetoothJXA}"
            if grep -Eqi 'not authorized|permission|denied|privacy' "$err" 2>/dev/null; then
                BLUETOOTH_SCAN_STATUS="AccessDenied"
                BLUETOOTH_SCAN_DETAIL="macOS denied Bluetooth access to osascript/CoreBluetooth. Allow Bluetooth access for the invoking terminal or management host, then rerun."
            else
                BLUETOOTH_SCAN_DETAIL="macOS CoreBluetooth JXA scan failed: $(tr '\r\n|' '   ' < "$err" 2>/dev/null | head -c 400)"
            fi
            ;;
    esac
}

bluetooth_map_to_tsv() {
    local path="$1"
    awk '
        BEGIN { RS="}" }

        function jstr(key, s,p,rest,out,i,c,esc) {
            s=$0
            p=match(s, "\\\"" key "\\\"[ \t]*:[ \t]*\\\"")
            if(!p) return ""
...

        {
            ...
            svc=jstr("ServiceUuid")
            mfr=jnum("ManufacturerId")
            if(mfr=="") mfr=jstr("ManufacturerId")
            name=jstr("Name")
            site=jstr("Site")

            printf "%s|%s|%s|%s|%s|%s|%s\n", address,svc,mfr,name,lat,lon,site
        }
    ' "$path"
}

match_known_bluetooth() {
    local match name address device_name anchor_rssi matched_by lat lon site count
    ...
    if [ -z "$BLUETOOTH_MAP_PATH" ]; then
        BLUETOOTH_LOCATION_MATCH="NotConfigured"
        return 1
    fi

    BLUETOOTH_LOCATION_MATCH="NoMatch"
    bluetooth_map_to_tsv "$BLUETOOTH_MAP_PATH" > "$BLUETOOTH_ANCHOR_FILE"
    [ -s "$BLUETOOTH_ANCHOR_FILE" ] || return 1
    [ -s "$BLUETOOTH_DEVICE_FILE" ] || return 1

    match="$(awk -F'|' '
        function normaddr(v){gsub(/[^0-9A-Fa-f]/,"",v); return toupper(v)}
        function hasmfr(list,val,a,n,i){
            n=split(list,a,";")
        ...
        NR==FNR { A[++ac]=$0; next }
        {
            ...
            for(i=1;i<=ac;i++) {
                ...
                if(x[1]!="" && normaddr(daddr)!="" && normaddr(x[1])==normaddr(daddr)) {
                    matched="Address"
                }
                else if(x[2]!="" && dsvc!="" && index(dsvc,toupper(x[2]))>0) {
                    matched="ServiceUuid"
                }
                else if(x[3]!="" && hasmfr(dmfr,x[3]) && x[4]!="" && dname==x[4]) {
                    matched="ManufacturerId+Name"
                }
                ...
                if(matched!="") {
                    ...
                    if(!found || drssi>best) {
                        found=1
                        best=drssi
                        bestline=x[4] "|" daddr "|" dname "|" drssi "|" matched "|" x[5] "|" x[6] "|" x[7]
                    }
                }
            }
        }
        END {
            if(found) print bestline "|" total
        }
    ' "$BLUETOOTH_ANCHOR_FILE" "$BLUETOOTH_DEVICE_FILE" 2>/dev/null || true)"

    [ -n "$match" ] || return 1

    IFS='|' read -r name address device_name anchor_rssi matched_by lat lon site count <<EOF_MATCH
$match
EOF_MATCH

    BLUETOOTH_ANCHOR_COUNT="${count:-1}"
    BLUETOOTH_ANCHOR_LAT="$lat"
    BLUETOOTH_ANCHOR_LON="$lon"
    BLUETOOTH_ANCHOR_SITE="$site"
    BLUETOOTH_ANCHOR_MATCHED_BY="$matched_by"
    BLUETOOTH_ANCHOR_RSSI="$anchor_rssi"
    BLUETOOTH_LOCATION_MATCH="${name:-$device_name}"
    BLUETOOTH_ANCHOR_MAP_URI="$(make_map_uri "$BLUETOOTH_ANCHOR_LAT" "$BLUETOOTH_ANCHOR_LON")"

    return 0
}

resolve_known_bluetooth() {
    if [ "$BLUETOOTH_ANCHOR_COUNT" -eq 0 ]; then
        match_known_bluetooth || return 1
    fi

    [ -n "$BLUETOOTH_ANCHOR_LAT" ] && [ -n "$BLUETOOTH_ANCHOR_LON" ] || return 1

    PHYSICAL_METHOD="KnownBluetooth"
    PHYSICAL_LAT="$BLUETOOTH_ANCHOR_LAT"
    PHYSICAL_LON="$BLUETOOTH_ANCHOR_LON"
    PHYSICAL_ACCURACY=""
    PHYSICAL_SITE="$BLUETOOTH_ANCHOR_SITE"
    PHYSICAL_VPN_RESISTANT="true"
    PHYSICAL_DETAIL="Known Bluetooth anchor matched [${BLUETOOTH_LOCATION_MATCH}] by ${BLUETOOTH_ANCHOR_MATCHED_BY}; anchorCount=${BLUETOOTH_ANCHOR_COUNT}; rssi=${BLUETOOTH_ANCHOR_RSSI} dBm. BLE discovery evidence only; no pairing, connection, authentication, or GATT interrogation was performed."
    return 0
}

bluetooth_display_name() {
    local address="$1" name="$2" addr_hex name_hex
    ...
    addr_hex="$(printf '%s' "$address" | tr -cd '[:xdigit:]' | tr '[:lower:]' '[:upper:]')"
    name_hex="$(printf '%s' "$name" | tr -cd '[:xdigit:]' | tr '[:lower:]' '[:upper:]')"

    # BlueZ commonly uses the address itself as Alias when an advertiser has no
    # local name. Rendering that as a second MAC column adds noise rather than
    # identity, so make the absence explicit while retaining the raw Alias/Name
    # in BLUETOOTH_DEVICE_FILE for matching and diagnostics.
    if [ ${#addr_hex} -eq 12 ] && [ "$name_hex" = "$addr_hex" ]; then
        printf '<unnamed>'
    else
        printf '%s' "$name"
    fi
...
bluetooth_compact_identity() {
    local company="$1" vendor="$2"
...
render_bluetooth_evidence() {
    local seconds="$1" address name rssi addr_type adv_type mfr_ids services seen source
    local primary company svc vendor conf display_address display_name identity
    local tmp rows_file rows=0 svc_present=0

    if [ ! -s "$BLUETOOTH_DEVICE_FILE" ]; then
        BLUETOOTH_EVIDENCE="Status=${BLUETOOTH_SCAN_STATUS}; Devices=0; ScanSeconds=${seconds}; Detail=${BLUETOOTH_SCAN_DETAIL}"
        return
    fi

    tmp="$WORK_DIR/bluetooth-evidence.txt"
    rows_file="$WORK_DIR/bluetooth-evidence-rows.txt"
    : > "$rows_file"

    while IFS='|' read -r address name rssi addr_type adv_type mfr_ids services seen source; do
        [ "$rows" -lt 25 ] || break
        rows=$((rows + 1))

        primary="$(bluetooth_primary_manufacturer_id "$mfr_ids")"
        company="$(bluetooth_company_name "$primary")"
        svc="$(bluetooth_service_summary "$services")"
        vendor="$(bluetooth_vendor_information "$address" "$addr_type" "$company")"
        conf="$(bluetooth_identity_confidence "$addr_type" "$name" "$mfr_ids" "$services" "$vendor")"
        identity="$(bluetooth_compact_identity "$company" "$vendor")"
        display_name="$(bluetooth_display_name "$address" "$name")"

        case "$address" in
            CBUUID:*) display_address="CB:$(printf '%s' "${address#CBUUID:}" | cut -c1-14)" ;;
            *) display_address="$address" ;;
        esac

        [ -n "$svc" ] && svc_present=1
        printf '%s|%s|%s|%s|%s|%s|%s\n' \
            "$rssi" "$display_address" "$display_name" "$seen" "$identity" "$conf" "$svc" >> "$rows_file"
    done < "$BLUETOOTH_DEVICE_FILE"

    if [ "$svc_present" -eq 1 ]; then
        printf '%4s %-17s %-30s %4s %-26s %-9s %-18s\n' \
            'RSSI' 'Address' 'Name' 'Seen' 'Identity' 'Conf' 'Services' > "$tmp"
        printf '%4s %-17s %-30s %4s %-26s %-9s %-18s\n' \
            '----' '-----------------' '------------------------------' '----' '--------------------------' '---------' '------------------' >> "$tmp"

        while IFS='|' read -r rssi display_address display_name seen identity conf svc; do
            printf '%4s %-17.17s %-30.30s %4s %-26.26s %-9.9s %-18.18s\n' \
                "$rssi" "$display_address" "$display_name" "$seen" "$identity" "$conf" "$svc" >> "$tmp"
        done < "$rows_file"
    else
        printf '%4s %-17s %-30s %4s %-26s %-9s\n' \
            'RSSI' 'Address' 'Name' 'Seen' 'Identity' 'Conf' > "$tmp"
        printf '%4s %-17s %-30s %4s %-26s %-9s\n' \
            '----' '-----------------' '------------------------------' '----' '--------------------------' '---------' >> "$tmp"

        while IFS='|' read -r rssi display_address display_name seen identity conf _; do
            printf '%4s %-17.17s %-30.30s %4s %-26.26s %-9.9s\n' \
                "$rssi" "$display_address" "$display_name" "$seen" "$identity" "$conf" >> "$tmp"
        done < "$rows_file"
    fi

    BLUETOOTH_EVIDENCE="$(printf 'Status=%s; Devices=%s; ScanSeconds=%s; Detail=%s\n' "$BLUETOOTH_SCAN_STATUS" "$BLUETOOTH_DEVICE_COUNT" "$seconds" "$BLUETOOTH_SCAN_DETAIL"; cat "$tmp")"
}

collect_bluetooth() {
    local seconds strongest
    [ "$BLUETOOTH_EXPLICIT" -eq 1 ] || return
    seconds="${BLUETOOTH_SCAN_SECONDS:-4}"

    if [ "$SKIP_BLUETOOTH" -eq 1 ]; then
        BLUETOOTH_SCAN_STATUS="SkippedByParameter"
        BLUETOOTH_SCAN_ENGINE="NotApplicable"
        BLUETOOTH_SCAN_DETAIL="Bluetooth scan skipped by parameter."
        BLUETOOTH_DEVICE_COUNT=0
        BLUETOOTH_EVIDENCE="Status=SkippedByParameter; Devices=0; ScanSeconds=${seconds}; Detail=${BLUETOOTH_SCAN_DETAIL}"
        return
    fi

    case "$PLATFORM" in
        Linux) scan_bluetooth_linux "$seconds" ;;
        macOS) scan_bluetooth_macos "$seconds" ;;
    esac

    BLUETOOTH_DEVICE_COUNT="$(awk 'END{print NR+0}' "$BLUETOOTH_DEVICE_FILE" 2>/dev/null || printf '0')"
    strongest="$(head -n 1 "$BLUETOOTH_DEVICE_FILE" 2>/dev/null || true)"
    if [ -n "$strongest" ]; then
        IFS='|' read -r BLUETOOTH_STRONGEST_ADDRESS BLUETOOTH_STRONGEST_NAME BLUETOOTH_STRONGEST_RSSI _ _ _ _ _ _ <<EOF_STRONG
$strongest
EOF_STRONG
    fi

    render_bluetooth_evidence "$seconds"
}
...
if [ "$BLUETOOTH_EXPLICIT" -eq 1 ]; then
    match_known_bluetooth || true
    verbose "Bluetooth: status=$BLUETOOTH_SCAN_STATUS; engine=$BLUETOOTH_SCAN_ENGINE; requestedSeconds=${BLUETOOTH_SCAN_SECONDS:-4}; devices=$BLUETOOTH_DEVICE_COUNT; anchors=$BLUETOOTH_ANCHOR_COUNT; locationMatch=$BLUETOOTH_LOCATION_MATCH."
fi
...
# Applicable macOS/Linux physical-location trust order aligned to the v3.62 evidence model:
# Known Wi-Fi -> Google Wi-Fi -> WiGLE -> Known Bluetooth anchor -> native OS provider -> public IP fallback.
...
if [ -n "$PHYSICAL_METHOD" ]; then
    ...
    :
elif resolve_known_bluetooth; then
    :
else
...
if [ -n "$PHYSICAL_METHOD" ] &&
   ...
   [ "$PHYSICAL_METHOD" != "GoogleWiFi" ] &&
   [ "$PHYSICAL_METHOD" != "WiGLEWiFi" ] &&
   [ "$PHYSICAL_METHOD" != "KnownBluetooth" ]; then
...
if [ "$PLATFORM" = "Linux" ] &&
   [ "$LINUX_NATIVE_PROVIDER" = "GeoClueUnavailable" ]; then
    add_note "${LINUX_GEOCLUE_UNAVAILABLE_DETAIL:-GeoClue where-am-i helper was not found.} Linux native location was unavailable; Wi-Fi resolvers and public-IP fallback remain available."
fi
...
verbose "Network context: collecting physical interface, gateway, and VPN evidence."
get_network_context
collect_vpn_evidence_v2
verbose "Router discovery: probing gateway ${DEFAULT_GATEWAY:-<none>} with bounded NAT-PMP/PCP/UPnP/HTTP/TLS checks."
discover_router_wan
...
assess_router_vpn
if [ "$TRACE_ENGINE" != "Unavailable" ]; then
    trace_row_count="$(awk 'END {print NR+0}' "$TRACE_FILE" 2>/dev/null || printf '0')"
    verbose "Route trace: engine=$TRACE_ENGINE; rows=$trace_row_count."
fi
...
case "$SELECTED_METHOD" in
    ...
    KnownBluetooth) BLUETOOTH_ANCHOR_MAP_URI="$SELECTED_MAP_URI" ;;
...
WIFI_VENDOR_OUI_COUNT="$(
    awk -F '|' '$7=="true" && $1!="" {split(toupper($1),a,":"); if (a[1]&&a[2]&&a[3]) print a[1] a[2] a[3]}' "$WIFI_FILE" 2>/dev/null |
    sort -u |
    awk 'END {print NR+0}'
)"
verbose "Wi-Fi evidence: rendering $NEARBY_WIFI_COUNT BSSID(s); up to ${WIFI_VENDOR_OUI_COUNT:-0} unique usable OUI vendor identities may require cache/API resolution."
WIFI_EVIDENCE="$(wifi_evidence_flat)"
verbose "Wi-Fi evidence: render complete."
...
if [ "$BLUETOOTH_EXPLICIT" -eq 1 ]; then
    if [ -n "$BLUETOOTH_ANCHOR_LAT" ] && [ -n "$BLUETOOTH_ANCHOR_LON" ] &&
       [ -n "$SELECTED_LAT" ] && [ -n "$SELECTED_LON" ]; then
        BLUETOOTH_LOCATION_DISTANCE="$(distance_km "$SELECTED_LAT" "$SELECTED_LON" "$BLUETOOTH_ANCHOR_LAT" "$BLUETOOTH_ANCHOR_LON")"
    else
        BLUETOOTH_LOCATION_DISTANCE="NotApplicable"
    fi

    if [ "$PLATFORM" = "macOS" ]; then
        print_field "BluetoothControllerStatus" "$BLUETOOTH_CONTROLLER_STATUS"
        print_field "BluetoothControllerSource" "$BLUETOOTH_CONTROLLER_SOURCE"
        print_field "BluetoothControllerPower" "$BLUETOOTH_CONTROLLER_POWER"
        print_field "BluetoothControllerName" "$BLUETOOTH_CONTROLLER_NAME"
        print_field "BluetoothControllerAddress" "$BLUETOOTH_CONTROLLER_ADDRESS"
        print_field "BluetoothControllerDetail" "${BLUETOOTH_CONTROLLER_DETAIL:-NotReported}"
    fi

    print_field "BluetoothScanStatus" "$BLUETOOTH_SCAN_STATUS"
    print_field "BluetoothScanEngine" "$BLUETOOTH_SCAN_ENGINE"
    print_field "BluetoothScanDetail" "$BLUETOOTH_SCAN_DETAIL"
    print_field "BluetoothDeviceCount" "$BLUETOOTH_DEVICE_COUNT"
    print_field "BluetoothAnchorCount" "$BLUETOOTH_ANCHOR_COUNT"
    print_field "StrongestBluetoothName" "${BLUETOOTH_STRONGEST_NAME:-NotReported}"
    print_field "StrongestBluetoothAddress" "${BLUETOOTH_STRONGEST_ADDRESS:-NotReported}"
    print_field "StrongestBluetoothRssi" "${BLUETOOTH_STRONGEST_RSSI:-NotReported}"
    print_field "BluetoothLocationMatch" "$BLUETOOTH_LOCATION_MATCH"
    print_field "BluetoothLocationDistanceKm" "$BLUETOOTH_LOCATION_DISTANCE"
fi
...
if [ "$BLUETOOTH_EXPLICIT" -eq 1 ]; then
    print_field "BluetoothAnchorMapUri" "${BLUETOOTH_ANCHOR_MAP_URI:-NotAvailable}"
